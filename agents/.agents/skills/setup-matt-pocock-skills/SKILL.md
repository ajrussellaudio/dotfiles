...
> Explainer: The "issue tracker" is where issues live for this repo. Skills like `to-tickets`, `triage`, `to-spec`, and `qa` read from and write to it — they need to know whether to call the Atlassian MCP against a Jira project, run `gh issue create`, write a markdown file under `.scratch/`, or follow some other workflow you describe. Pick the place you actually track work for this repo — which is not necessarily where the code is hosted.

Default posture: **work repos track their issues in Jira, and GitHub Issues are usually disabled there** — so a GitHub remote alone doesn't settle this. Propose Jira when exploration finds a Jira signal (Jira issue keys like `ABC-1234` in `git log` or branch names, an Atlassian MCP server configured, a `.jira` config). Otherwise propose the tracker matching the `git remote` — GitHub for a GitHub remote, GitLab for `gitlab.com` or a self-hosted GitLab host. Offer:
